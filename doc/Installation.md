...
This document describes how to build, install and upgrade the bumblebee.
...
[bumblebee-*.deb](https://github.com/gotthardp/bumblebee/releases)
and install it by:
```bash
dpkg -i bumblebee-<VERSION>.deb
```

If you want the server to start automatically after system reboot, run
`systemctl enable bumblebee`.

Then start the server by `systemctl start bumblebee`.
...
[bumblebee-*.tar.gz](https://github.com/gotthardp/bumblebee/releases)
and (on Linux) unpack it by:
```bash
mkdir bumblebee
mv bumblebee-<VERSION>.tar.gz bumblebee/
cd bumblebee
tar -zxvf bumblebee-<VERSION>.tar.gz
```
 ...
 * Unpack the binary release to `/usr/lib/bumblebee`
 * Copy `bin/bumblebee.service` to `/lib/systemd/system`
 * Create a dedicated user by `useradd --home-dir /var/lib/bumblebee --create-home lorawan`
 * Create the `/var/log/bumblebee` directory and make sure it is owned by the user `lorawan`
 * Start the server by `systemctl start bumblebee`

This will put the database into `/var/lib/bumblebee` and server logs into
`/var/log/bumblebee`.
...
(e.g. `bumblebee`) and then run the server by double-clicking `bumblebee.bat`
in the `bumblebee/bin` folder.

You can also run the bumblebee as a Windows service.
 ...
 * In `/usr/lib/bumblebee` when using the official releases
 * In `bumblebee/_build/default/rel/bumblebee` when you build the server
   from sources
 ...
 * Delete the existing `bin`, `lib` and `releases` sub-directories and replace
   ...
   unpack the content of the binary release e.g. in `/usr/lib/bumblebee`.
 * Copy `bin/bumblebee.service` to `/lib/systemd/system`
...
Get the latest bumblebee sources by:
```bash
...
Then build and release the bumblebee by:
```bash
make release
```

The release will be created in `bumblebee/_build/default/rel/bumblebee`.
...
`/usr/lib/bumblebee`. To upgrade your installation you shall **replace** the
...
`bumblebee/_build/default/rel/bumblebee/bumblebee_<VERSION>.deb`.
...
You can start the server by `systemctl start bumblebee`.
