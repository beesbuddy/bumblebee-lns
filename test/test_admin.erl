...
-export([add_area/1, add_gateway/2, add_network/1, add_group/2, add_profile/2, add_node/2, add_device/2]).
...
add_network(NetName) ->
    post_json("networks", [{name, NetName}, {netid, <<"000000">>},
        ...
        {join1_delay, 1}, {join2_delay, 2}, {rx1_delay, 1}, {rx2_delay, 2},
...
add_profile(GroupName, ProfName) ->
    post_json("profiles", [{name, ProfName}, {group, GroupName}, {app, <<"semtech-mote">>},
        {join, 1}, {fcnt_check, undefined}, {adr_mode, 0}]).
...
add_device(ProfName, {DevEUI, AppEUI, AppKey}) ->
    post_json("devices", [{deveui, lorawan_utils:binary_to_hex(DevEUI)}, {profile, ProfName},
        {appeui, lorawan_utils:binary_to_hex(AppEUI)}, {appkey, AppKey}]).
...
request_with_retry(Uri, Body, Retries) ->
    case httpc:request(post, {test_env:base_url() ++ "/api/" ++ Uri,
            [{"Authorization", basic_auth()}],
            "application/json", jsx:encode(Body)}, [], []) of
