...
configure_ports() ->
    ...
    application:set_env('bumblebee_lns', http_admin_listen, [{port, HttpPort}]),
    application:set_env('bumblebee_lns', http_admin_listen_ssl, []),
    application:set_env('bumblebee_lns', http_admin_redirect_ssl, false),
    application:set_env('bumblebee_lns', packet_forwarder_listen, [{port, UdpPort}]),
    application:set_env('bumblebee_lns', test_http_port, HttpPort),
    application:set_env('bumblebee_lns', test_udp_port, UdpPort),
    ok.
...
gateway_server() ->
    {ok, Port} = application:get_env('bumblebee_lns', test_udp_port),
    {{127, 0, 0, 1}, Port}.

http_port() ->
    {ok, Port} = application:get_env('bumblebee_lns', test_http_port),
    Port.
...
wait_http_ready(Retries) ->
    case listener_port() of
        {ok, Port} ->
            application:set_env('bumblebee_lns', test_http_port, Port),
