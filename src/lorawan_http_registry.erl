...
get_static(routes) ->
    [
        ...
        {"/api/scopes/[:name]", lorawan_admin_scopes, [{<<"server:read">>, '*'}]},
        ...
        {"/api/servers/[:sname]", lorawan_admin_servers,
            {[{<<"server:read">>, '*'}], [{<<"server:write">>, '*'}]}},
        {"/api/applications/[:name]", lorawan_admin_applications, [{<<"web-admin">>, '*'}]},
        ...
        {"/api/profiles/[:name]", lorawan_admin_db_record,
            {profile, record_info(fields, profile),
                {[{<<"device:read">>, '*'}], [{<<"device:write">>, '*'}]}}},
        {"/api/choices/regions", lorawan_admin_choices, {regions, [{<<"web-admin">>, '*'}]}},
        {"/api/choices/groups", lorawan_admin_choices, {groups, [{<<"web-admin">>, '*'}]}},
        {"/api/choices/profiles", lorawan_admin_choices, {profiles, [{<<"web-admin">>, '*'}]}},
        ...
        {"/api/connectors/[:connid]", lorawan_admin_db_record,
            {connector, record_info(fields, connector),
                {[{<<"backend:read">>, '*'}], [{<<"backend:write">>, '*'}]}}},
        {"/api/connections/[:app]", lorawan_admin_connections, [{<<"backend:read">>, '*'}]},
        ...
        {"/api/events/[:evid]", lorawan_admin_db_record,
            {event, record_info(fields, event),
                {[{<<"device:read">>, '*'}], [{<<"device:write">>, '*'}]}}},
        {"/api/upload", lorawan_admin_upload, {[], [{<<"backend:write">>, '*'}]}},
        {"/admin", lorawan_admin_static,
            {priv_file, bumblebee, <<"admin/index.html">>, [{<<"web-admin">>, '*'}]}},
        {"/admin/timeline", lorawan_admin_timeline, [{<<"web-admin">>, '*'}]},
        {"/admin/admin-config.js", lorawan_admin_config_js, [{<<"web-admin">>, '*'}]},
        {"/admin/sgraph/:sname", lorawan_admin_graph_server, [{<<"server:read">>, '*'}]},
        {"/admin/pgraph/:mac", lorawan_admin_graph_gw, {pgraph, [{<<"network:read">>, '*'}]}},
        {"/admin/tgraph/:mac", lorawan_admin_graph_gw, {tgraph, [{<<"network:read">>, '*'}]}},
        {"/admin/rgraph/:devaddr", lorawan_admin_graph_rx, {rgraph, [{<<"device:read">>, '*'}]}},
        {"/admin/qgraph/:devaddr", lorawan_admin_graph_rx, {qgraph, [{<<"device:read">>, '*'}]}},
        {"/admin/ngraph/:devaddr", lorawan_admin_graph_node, [{<<"device:read">>, '*'}]},
        {"/admin/[...]", lorawan_admin_static,
            {priv_dir, bumblebee, <<"admin">>, [{<<"web-admin">>, '*'}]}}
    ].
...
custom_web([{URL, dir, Path, Scope} | Dirs]) ->
    [
        {URL, lorawan_admin_static, {dir, Path, Scope}}
        | custom_web(Dirs)
    ];
custom_web([{URL, file, Path, Scope} | Dirs]) ->
    [
        {URL, lorawan_admin_static, {file, Path, Scope}}
        | custom_web(Dirs)
    ];
custom_web([]) ->
