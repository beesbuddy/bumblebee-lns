...
uplink(MAC) ->
    gen_server:cast(?MODULE, {uplink, bumblebee_utils:binary_to_hex(MAC)}).

downlink(MAC) ->
    gen_server:cast(?MODULE, {downlink, bumblebee_utils:binary_to_hex(MAC)}).
...
handle_cast({warning, Source, Text, Num}, State) ->
    ...
    prometheus_counter:inc(bumblebee_warnings_total, [Entity, Eid, Text], Num),
    {noreply, State};
handle_cast({error, Source, Text, Num}, State) ->
    {Entity, Eid} = expand_source(Source),
    prometheus_counter:inc(bumblebee_errors_total, [Entity, Eid, Text], Num),
    {noreply, State};
handle_cast({uplink, MAC}, State) ->
    prometheus_counter:inc(bumblebee_uplinks_total, [MAC], 1),
    {noreply, State};
handle_cast({downlink, MAC}, State) ->
    prometheus_counter:inc(bumblebee_downlinks_total, [MAC], 1),
    {noreply, State}.
...
expand_source({Other, Eid}) -> {Other, bumblebee_utils:binary_to_hex(Eid)};
% uncaught in bumblebee_utils:throw_event/3-4
