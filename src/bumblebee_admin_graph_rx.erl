...
%
-module(bumblebee_admin_graph_rx).
...
is_authorized(Req, #state{scopes = Scopes} = State) ->
    case bumblebee_admin:handle_authorization(Req, Scopes) of
        {true, AuthFields} ->
...
forbidden(Req, #state{auth_fields = AuthFields} = State) ->
    {bumblebee_admin:fields_empty(AuthFields), Req, State}.
...
get_rxframe(Req, State) ->
    ...
    ActRec = get_last_rxframes(bumblebee_utils:hex_to_binary(DevAddr)),
    case ActRec of
...
get_last_rxframes(DevAddr) ->
    {Uplinks0, _} = bumblebee_db:get_rxframes(DevAddr),
    Uplinks =
        if
            length(Uplinks0) =< 50 ->
                Uplinks0;
...
send_array(
    Req,
    #network{region = Region, max_eirp = MaxEIRP},
    DevAddr,
    ActRec,
    #state{format = rgraph} = State
) ->
    #{min := Min, max := Max} = bumblebee_mac_region:freq(Region),
    ...
    Array = [
        ...
        {rows,
            lists:map(
                fun
                    (#rxframe{fcnt = FCnt, powe = TXPower, gateways = Gateways}) when
                        is_number(TXPower), is_binary(Region)
                    ->
                        {_MAC, #rxq{datr = DatR, freq = Freq}} = hd(Gateways),
                        [
                            {c, [
                                [{v, FCnt}],
                                [{v, bumblebee_mac_region:datar_to_dr(Region, DatR)}, {f, DatR}],
                                [
                                    {v, MaxEIRP / 2 - TXPower},
                                    {f, integer_to_binary(MaxEIRP - 2 * TXPower)}
                                ],
                                [{v, Freq}]
                            ]}
                        ];
    ...
    {
        jsx:encode([
            {devaddr, DevAddr}, {array, Array}, {'band', [{minValue, Min}, {maxValue, Max}]}
        ]),
        Req,
        State
    };
send_array(Req, #network{}, DevAddr, ActRec, #state{format = qgraph} = State) ->
    Array = [
        ...
        {rows,
            lists:map(
                fun
                    (#rxframe{fcnt = FCnt, gateways = Gateways, average_qs = AverageQs}) ->
...
resource_exists(Req, State) ->
    case
        mnesia:dirty_read(
            node,
            bumblebee_admin:parse_field(devaddr, cowboy_req:binding(devaddr, Req))
        )
    of
        [] -> {false, Req, State};
