...
%
-module(bumblebee_admin_connections).
...
handle_get(Req, #state{app = undefined} = State) ->
    Filter = bumblebee_admin:parse(get_filters(Req)),
    Items =
        lists:filter(
            fun(Conn) ->
                filter_matches(Conn, Filter)
            end,
            get_connections(bumblebee_compat:pg_which_groups(), [])
        ),
...
get_connection(App) ->
    case bumblebee_compat:pg_get_members({backend, App}) of
...
handle_action(Req, #state{app = App, action = <<"send">>} = State) ->
    case bumblebee_backend_factory:nodes_with_backend(App) of
        [{Profile, #node{devaddr = DevAddr, appargs = AppArgs} = Node} | _] ->
            ...
            lager:debug("Sending connector test ~p to ~p", [
                App, bumblebee_utils:binary_to_hex(DevAddr)
            ]),
            bumblebee_backend_factory:event(App, {Profile, Node}, Vars);
        [] ->
