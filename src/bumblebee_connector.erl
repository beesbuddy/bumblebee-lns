...
build_access_token(Res0, AccessKey, Expiry) ->
    Res = bumblebee_compat:uri_encode(Res0),
    ...
    Sig = bumblebee_compat:uri_encode(
        base64:encode_to_string(
            bumblebee_compat:hmac_sha256(base64:decode(AccessKey), ToSign)
        )
    ),
...
form_encode(Values) ->
    cow_qs:qs(
        lists:map(
            ...
            maps:to_list(bumblebee_admin:build(Values))
        )
    ).
...
decode_and_downlink(#connector{app = App, format = Format}, Msg, Bindings) ->
    case decode(Format, Msg) of
        {ok, Vars} when is_map(Vars) ->
            bumblebee_application_backend:handle_downlink(
                App,
...
decode(<<"raw">>, Msg) ->
    {ok, #{data => Msg}};
decode(<<"json">>, Msg) ->
    case catch bumblebee_admin:parse(jsx:decode(Msg, [return_maps, {labels, atom}])) of
        {'EXIT', Error} ->
...
raise_failed(ConnId, {Error, Args}) ->
    bumblebee_utils:throw_error({connector, ConnId}, {Error, Args}),
    {atomic, ok} = append_failed(ConnId, Error);
raise_failed(ConnId, Error) ->
    bumblebee_utils:throw_error({connector, ConnId}, Error),
    {atomic, ok} = append_failed(ConnId, Error).
...
append_failed(ConnId, Error) ->
    mnesia:transaction(
        fun() ->
            ...
            bumblebee_admin:write(append_failed0(Rec, atom_to_binary(Error, latin1)))
        end
    ).
