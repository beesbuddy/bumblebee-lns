...
%
-module(bumblebee_application_backend).
-behaviour(bumblebee_application).
...
handle_uplink(
    {Network, #profile{app = AppID} = Profile, #node{devaddr = DevAddr} = Node},
    _RxQ,
    LastMissed,
    Frame
) ->
    case mnesia:dirty_read(handler, AppID) of
        [#handler{downlink_expires = Expires} = Handler] ->
            case LastMissed of
                {missed, _Receipt} ->
                    case bumblebee_application:get_stored_frames(DevAddr) of
                        [] ->
                            retransmit;
...
handle_rxq(
    {_Network, _Profile, #node{devaddr = DevAddr}},
    _Gateways,
    _WillReply,
    #frame{port = Port},
    undefined
...
handle_rxq(
    {_Network, #profile{app = AppID} = Profile, #node{devaddr = DevAddr} = Node},
    Gateways,
    _WillReply,
...
parse_uplink(
    #handler{app = AppName, payload = Payload, uplink_fields = Fields},
    {#network{netid = NetID}, #profile{appid = AppID}, #node{
        appargs = AppArgs, desc = Desc, devstat = DevStat
    }},
    ...
    RxQ =
        lists:map(
            fun({MAC, #rxq{time = Time, tmms = TmMs, rssi = RSSI, lsnr = SNR}}) ->
                vars_add(
                    desc,
                    get_dbfield(gateway, MAC, #gateway.desc),
                    Fields,
                    vars_add(
                        gpsalt,
                        get_dbfield(gateway, MAC, #gateway.gpsalt),
                        Fields,
                        vars_add(
                            gpspos,
                            get_dbfield(gateway, MAC, #gateway.gpspos),
                            Fields,
...
vars_add(_Field, Value, Fields, Vars) when
    Value == undefined; Fields == undefined
->
    Vars;
...
send_event(
    Event, Vars0, #handler{app = AppName, event_fields = Fields, parse_event = Parse}, DeviceOrNode
) ->
    Vars =
        vars_add(
            app,
            ...
            vars_add(
                ...
                vars_add(
                    ...
                    case DeviceOrNode of
                        {
                            #profile{appid = AppID},
                            #device{deveui = DevEUI, appargs = AppArgs, desc = Desc},
                            DevAddr
                        } ->
                            vars_add(
                                appid,
                        ...
                        {#profile{appid = AppID}, #node{
                            devaddr = DevAddr, appargs = AppArgs, desc = Desc
                        }} ->
                            vars_add(
                                appid,
                                AppID,
    ...
    bumblebee_backend_factory:event(
        AppName,
        DeviceOrNode,
...
fields_to_data(AppName, {_, Fun}, Vars) when is_function(Fun) ->
    try
        ...
        Error:Term ->
            bumblebee_utils:throw_error({handler, AppName}, {build_failed, {Error, Term}}),
            % no downlink
            []
    end;
...
fields_to_data(AppName, _Else, _Vars) ->
    bumblebee_utils:throw_error({handler, AppName}, json_syntax_error),
    % no downlink
    [].
...
send_downlink(Handler, #{deveui := DevEUI}, undefined, TxData) ->
    case mnesia:dirty_read(device, DevEUI) of
        ...
        [Device] ->
            ...
            bumblebee_application:store_frame(Device#device.node, TxData)
    end;
...
send_downlink(Handler, #{devaddr := DevAddr}, undefined, TxData) ->
    case mnesia:dirty_read(node, DevAddr) of
        ...
        [Node] ->
            % standard downlink to an explicit node
            purge_frames(Handler, Node, TxData),
            bumblebee_application:store_frame(DevAddr, TxData)
    end;
    ...
    case mnesia:dirty_read(node, DevAddr) of
        [] ->
            case mnesia:dirty_read(multicast_channel, DevAddr) of
                ...
                [Group] ->
                    % scheduled multicast
                    bumblebee_handler:multicast(Group, Time, TxData)
            end;
        [Node] ->
            % class C downlink to an explicit node
            try_class_c(Handler, Node, Time, TxData)
...
send_downlink(Handler, #{app := AppID}, undefined, TxData) ->
    ...
    filter_group_responses(
        ...
        lists:map(
            ...
            bumblebee_backend_factory:nodes_with_backend(AppID)
        )
    );
...
try_class_c(
    ...
    if
        SecSinceLast < Delay ->
            lager:debug("~s delaying Class C downlink", [bumblebee_utils:binary_to_hex(DevAddr)]),
            % the node recently sent a Class A uplink
            {ok, _} = timer:apply_after(Delay * 1000, ?MODULE, send_class_c, [
                {Network, Profile, Node}, Handler, Time, TxData
            ]),
            ok;
        true ->
            send_class_c({Network, Profile, Node}, Handler, Time, TxData)
...
send_class_c({Network, Profile, Node}, Handler, Time, TxData) ->
    purge_frames(Handler, Node, TxData),
    bumblebee_handler:downlink({Network, Profile, Node}, Time, TxData).

purge_frames(
    #handler{downlink_expires = <<"superseded">>} = Handler,
    #node{devaddr = DevAddr} = Node,
    #txdata{port = Port}
) ->
    lists:foreach(
        fun
            (#txdata{confirmed = true, receipt = Receipt}) ->
                ...
                send_event(lost, #{receipt => Receipt}, Handler, {
                    bumblebee_db:get_profile(Node), Node
                });
            (#txdata{}) ->
                ok
        end,
        bumblebee_application:take_previous_frames(DevAddr, Port)
    );
...
% accelerometer
cayenne_decode(
    <<Ch, 113, X:16/signed-integer, Y:16/signed-integer, Z:16/signed-integer, Rest/binary>>, Acc
) ->
...
% gyrometer
cayenne_decode(
    <<Ch, 134, X:16/signed-integer, Y:16/signed-integer, Z:16/signed-integer, Rest/binary>>, Acc
) ->
    cayenne_decode(Rest, add_field(Ch, #{x => X / 100, y => Y / 100, z => Z / 100}, Acc));
% gps
cayenne_decode(
    <<Ch, 136, Lat:24/signed-integer, Lon:24/signed-integer, Alt:24/signed-integer, Rest/binary>>,
    Acc
) ->
    cayenne_decode(
        Rest, add_field(Ch, #{lat => Lat / 10000, lon => Lon / 10000, alt => Alt / 100}, Acc)
    );
cayenne_decode(<<>>, Acc) ->
    Acc.
...
cayenne_test_() ->
    [
        ?_assertEqual(
            #{<<"field3">> => 27.2, <<"field5">> => 25.5},
            cayenne_decode(bumblebee_utils:hex_to_binary(<<"03670110056700FF">>))
        ),
        ?_assertEqual(
            #{<<"field1">> => -4.1},
            cayenne_decode(bumblebee_utils:hex_to_binary(<<"0167FFD7">>))
        ),
        ?_assertEqual(
            #{<<"field6">> => #{x => 1.234, y => -1.234, z => 0.0}},
            cayenne_decode(bumblebee_utils:hex_to_binary(<<"067104D2FB2E0000">>))
        ),
        ?_assertEqual(
            #{<<"field1">> => #{lat => 42.3519, lon => -87.9094, alt => 10.0}},
            cayenne_decode(bumblebee_utils:hex_to_binary(<<"018806765ff2960a0003e8">>))
        )
    ].
