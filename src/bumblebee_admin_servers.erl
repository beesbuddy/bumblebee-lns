...
%
-module(bumblebee_admin_servers).
...
init(Req, Scopes) ->
    Key = bumblebee_admin:parse_field(sname, cowboy_req:binding(sname, Req)),
...
get_server(Node) ->
    case lists:member(Node, nodes([this, connected])) of
        ...
        false ->
            Config = bumblebee_admin:build(?to_map(server, load_server(Node))),
            Config#{
...
write_server(Req, #server{sname = NodeName} = Server, State) ->
    case bumblebee_db:join_cluster(node(), NodeName) of
        ok ->
...
delete_resource(Req, #state{key = NodeName} = State) ->
    case bumblebee_db:leave_cluster(NodeName) of
        ok ->
