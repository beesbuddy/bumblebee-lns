...
init(Req, []) ->
    MAC = bumblebee_admin:parse_field(mac, cowboy_req:binding(mac, Req)),
            ...
            case mnesia:dirty_read(area, Name) of
                [#area{} = Area] ->
                    {cowboy_websocket, Req, State#state{
                        mac = MAC, area = Area, pending = maps:new()
                    }};
                _ ->
                    bumblebee_utils:throw_error({gateway, MAC}, unknown_area, aggregated),
                    error_response(500, Req)
            end;
        _Else ->
            bumblebee_utils:throw_error({gateway, MAC}, unknown_mac, aggregated),
        ...
        {basic, User, Pass} ->
            case mnesia:dirty_read(user, User) of
                [#user{pass_ha1 = HA1, scopes = AuthScopes}] ->
                    case bumblebee_http_digest:ha1({User, ?REALM, Pass}) of
                        HA1 ->
                            {ok, User, AuthScopes};
                        _Else ->
                            unauthorized
...
handle_message(#{router := Router}, #state{mac = undefined} = State) ->
    MAC = bumblebee_eid:parse(Router),
    ID6 = bumblebee_eid:as_id6(MAC),
    % determine own address
    [#config{admin_url = Prefix}] = mnesia:dirty_read(config, <<"main">>),
    {ok, {Scheme, _, Host, Port, Path, _}} = bumblebee_compat:parse_uri(Prefix, [
        {http, 80}, {https, 443}
    ]),
    Scheme2 =
        case Scheme of
    ...
    Res = #{
        ...
        uri => list_to_binary([
            ...
            Path,
            <<"router-info/">>,
            bumblebee_utils:binary_to_hex(MAC)
        ])
    },
    {reply, Res, State};
handle_message(#{router := _Router}, #state{mac = MAC}) ->
    _ = lager:error("Unexpected router-info from ~s", [bumblebee_utils:binary_to_hex(MAC)]),
    stop;
handle_message(
    #{
        msgtype := <<"version">>,
    ...
    _ = lager:debug("~s basicstation ~s ~s ~s ~s", [
        bumblebee_utils:binary_to_hex(MAC), Station, Firmware, Package, Model
    ]),
    bumblebee_gw_router:alive(MAC, self(), {IP, Port, Proto}),
...
handle_message(
    ...
    AppEUI = bumblebee_utils:reverse(bumblebee_eid:parse(JoinEUI0)),
    DevEUI = bumblebee_utils:reverse(bumblebee_eid:parse(DevEUI0)),
    PHYPayload =
        <<MHdr, AppEUI:8/binary, DevEUI:8/binary, DevNonce:16/little-unsigned-integer,
            MIC:32/little-signed-integer>>,
    bumblebee_gw_router:uplinks([{{MAC, RxQ, GWState}, PHYPayload}]);
handle_message(
    #{
        msgtype := <<"updf">>,
        'MHdr' := MHdr,
    ...
    PHYPayload =
        <<MHdr, DevAddr:32/little-signed-integer, FCtrl, FCnt:16/little-unsigned-integer,
            (bumblebee_utils:hex_to_binary(FOpts))/binary, FPort/binary,
            (bumblebee_utils:hex_to_binary(FRMPayload))/binary, MIC:32/little-signed-integer>>,
    bumblebee_gw_router:uplinks([{{MAC, RxQ, GWState}, PHYPayload}]);
handle_message(#{msgtype := <<"propdf">>, 'FRMPayload' := FRMPayload}, #state{mac = MAC}) ->
    lager:debug("~s got proprietary frame ~s", [
        bumblebee_utils:binary_to_hex(MAC), bumblebee_utils:binary_to_hex(FRMPayload)
    ]);
...
websocket_info(
    {send, _, GWState, DevAddr, #txq{freq = Freq, datr = DatR, time = Time}, _, PHYPayload},
    ...
    Reply0 = #{
        ...
        'DevEui' => bumblebee_utils:binary_to_hex(<<0:32, DevAddr/binary>>),
        diid => DIID,
        pdu => bumblebee_utils:binary_to_hex(PHYPayload),
        'DR' => bumblebee_mac_region:datar_to_dr(Region, DatR),
        'Freq' => round(Freq * ?MHZ),
        priority => 0,
        'MuxTime' => bumblebee_utils:time_to_unix() / 1000
    },
    Reply =
        case Time of
            % class A
    ...
    %% lager:debug("LNS <- ~p", [Reply]),
    {reply, {text, jsx:encode(Reply)}, State#state{pending = maps:put(DIID, Timer, Pending)}};
websocket_info({no_ack, DIID, DevAddr}, #state{mac = MAC, pending = Pending} = State) ->
    case maps:take(DIID, Pending) of
        {_, Pending2} ->
            bumblebee_gw_router:downlink_error(MAC, DevAddr, {not_transmitted, DIID}),
            {ok, State#state{pending = Pending2}};
        error ->
            ok
    end;
...
router_config(#area{region = Region}, MAC) ->
    #{min := MinFreq, max := MaxFreq} = bumblebee_mac_region:freq(Region),
    {atomic, {NetIDs, Freq, MaxDR}} =
        mnesia:transaction(fun() ->
            mnesia:foldl(
                fun
                    (
                        #network{netid = <<NetID:24>>, region = Reg, max_datr = Max2} = Net,
                        {IDs, Freq1, Max1}
                    ) when
                        Reg == Region
                    ->
                        Freq2 = bumblebee_mac_region:net_freqs(Net),
    ...
    lager:debug("gateway ~s init radio_0: ~p, radio_1: ~p", [
        bumblebee_utils:binary_to_hex(MAC), Ra1Fr, Ra2Fr
    ]),
    #{
        ...
        'DRs' => lists:filtermap(
            fun
                ({Num, {SF, BW}, Dir}) when Num =< MaxDR ->
                    {true, [SF, BW, dir_to_dnonly(Dir)]};
                ({Num, BW, Dir}) when Num =< MaxDR ->
                    % FSK
                    {true, [0, BW, dir_to_dnonly(Dir)]};
                (_) ->
                    false
            end,
            bumblebee_mac_region:datars(Region)
        ),
        sx1301_conf => [
            maps:from_list(
...
handle_reftime(MAC, #{'RefTime' := RefTime}) when RefTime > 1000000 ->
    bumblebee_gw_router:network_delay(MAC, bumblebee_utils:time_to_unix() - RefTime * 1000);
handle_reftime(_, _) ->
    ok.
...
parse_rxq(Region, #{
...
}) ->
    {
        #rxq{
            freq = Freq / ?MHZ,
            datr = bumblebee_mac_region:dr_to_datar(Region, DR),
            codr = <<"4/5">>,
            tmms =
                if
                    Time > 0 -> Time / 1000;
