...
%
-module(bumblebee_application_semtech_mote).
...
handle_rxq(
    ...
    _WillReply,
    #frame{
        port = 2, data = <<LED, Press:16, Temp:16, _AltBar:16, Batt, _Lat:24, _Lon:24, _AltGps:16>>
    },
    []
) ->
    % this is used in CN779, EU868, IN865, KR920
    lager:debug("PUSH_DATA ~s ~w ~w ~w", [bumblebee_utils:binary_to_hex(DevAddr), Press, Temp, Batt]),
...
handle_rxq(
    ...
    lager:debug("PUSH_DATA ~s ~w ~w", [bumblebee_utils:binary_to_hex(DevAddr), Temp, Batt]),
    % blink with the LED indicator
    {send, #txdata{port = 2, data = <<((LED + 1) rem 2)>>}};
...
handle_rxq(
    ...
    lager:debug("PUSH_DATA ~s ignore empty frame", [bumblebee_utils:binary_to_hex(DevAddr)]),
    ok;
