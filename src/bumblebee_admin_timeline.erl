...
%
-module(bumblebee_admin_timeline).
    ...
    Events = lists:map(
        fun
            (
                ...
                [
                    {id, bumblebee_utils:binary_to_hex(Id)},
                    {className, Severity},
                    {content, Text},
...
addr_port(DevAddr, Port) ->
    <<(bumblebee_utils:binary_to_hex(DevAddr))/binary, ":", (integer_to_binary(Port))/binary>>.
...
title(#event{entity = Entity, eid = EID} = Event) ->
    [io_lib:print(Entity), " ", bumblebee_utils:binary_to_hex(EID), "<br\>", title0(Event)].
