...
%
-module(bumblebee_connector_mqtt).
...
start_connector(#connector{connid = Id} = Connector) ->
    bumblebee_connector_sup:start_child({mqtt, Id}, ?MODULE, [Connector]).

stop_connector(Id) ->
    bumblebee_connector_sup:stop_child({mqtt, Id}).
...
init([
    ...
    try
        {ok, #state{
            conn = Connector,
            connect = bumblebee_connector:prepare_filling([Uri, ClientId, UserName, Password]),
            subscribe = bumblebee_connector:prepare_filling(Sub),
            publish_uplinks = bumblebee_connector:prepare_filling(PubUp),
            publish_events = bumblebee_connector:prepare_filling(PubEv),
            received = bumblebee_connector:prepare_matching(Cons),
            hier = []
        }}
    catch
        _:Error:Stack ->
            lager:debug("connector ~s failed: ~p~n~p", [Id, Error, Stack]),
            bumblebee_connector:raise_failed(Id, Error),
            {stop, shutdown}
    end.
...
build_hierarchy(PatConn, PatSub, Nodes) ->
    lists:foldl(
        fun(Node, Hier) ->
            Vars = bumblebee_admin:build(bumblebee_connector:node_to_vars(Node)),
            #{devaddr := DevAddr} = Vars,
            Connect = bumblebee_connector:fill_pattern(PatConn, Vars),
            {Subs, Nos} =
            ...
            Subs2 =
                if
                    PatSub == undefined; PatSub == ?EMPTY_PATTERN ->
                        Subs;
                    true ->
                        Subscribe = bumblebee_connector:fill_pattern(PatSub, Vars),
                ...
                case not lists:keymember(Connect, 1, CurrHier) of
                    true ->
                        ...
                        case connect([v5, v4, v3], Connect, Conn) of
                            ...
                            {error, Error} ->
                                bumblebee_connector:raise_failed(Id, {badarg, Error}),
                                false
                        end;
                    false ->
                        % already processed
    ...
    try connection_args(Arguments, Conn) of
        CArgs ->
            case connect0(Vers, CArgs) of
                {ok, _Ver, C} ->
                    {ok, C, #costa{
                        cargs = CArgs, connect_count = 0, last_connect = calendar:universal_time()
                    }};
                {error, Error} ->
                    {error, Error}
            end
...
connection_args([Uri, ClientId, UserName, Password], Conn) ->
    lager:debug("Connecting ~s to ~p, id ~p, user ~p", [
        Conn#connector.connid, Uri, ClientId, UserName
    ]),
    {ok, ConnUri} = bumblebee_compat:parse_uri(Uri, [{mqtt, 1883}, {mqtts, 8883}]),
...
handle_info(
    ...
    NewH = build_hierarchy(
        PatConn,
        PatSub,
        bumblebee_backend_factory:nodes_with_backend(Connector#connector.app)
    ),
    Hier2 =
...
handle_info(
    ...
    {noreply, State};
handle_info(
    {publish, #{topic := Topic, payload := Payload, client_pid := C}},
    State = #state{conn = Connector, received = Pattern, hier = Hier}
) ->
    Vars = bumblebee_connector:match_vars(Topic, Pattern),
    Vars2 =
        case
            lists:any(
    ...
    case bumblebee_connector:decode_and_downlink(Connector, Payload, Vars2) of
        ok ->
            ok;
        {error, {Object, Error}} ->
...
handle_info({'EXIT', C, Error}, #state{conn = #connector{connid = ConnId}, hier = Hier} = State) ->
    case lists:keyfind(C, 2, Hier) of
        {Connect, C, Subs, #costa{connect_count = Count} = Costa} ->
            lager:debug("Connector ~p to ~p failed: ~p (count: ~p)", [
                ConnId, hd(Connect), Error, Count
            ]),
...
terminate(Reason, #state{conn = #connector{connid = ConnId}}) when
...
handle_reconnect(ConnId, Connect, #costa{last_connect = Last, connect_count = Count} = Costa) ->
    case
        calendar:datetime_to_gregorian_seconds(calendar:universal_time()) -
            calendar:datetime_to_gregorian_seconds(Last)
    of
        Diff when Diff < 30, Count > 120 ->
            % give up after 2 hours
            bumblebee_connector:raise_failed(ConnId, network),
            remove;
...
connection_for_node(Node, #state{connect = PatConn, hier = Hier}) ->
    Connect = bumblebee_connector:fill_pattern(
        PatConn,
        bumblebee_admin:build(bumblebee_connector:node_to_vars(Node))
    ),
    case lists:keyfind(Connect, 1, Hier) of
...
publish_uplink(C, PatPub, Format, QoS, Vars) ->
    Topic = bumblebee_connector:fill_pattern(PatPub, bumblebee_admin:build(Vars)),
    QoS0 = default(QoS, 0),
...
publish_event(C, PatPub, QoS, Vars0) ->
    Vars = bumblebee_admin:build(Vars0),
    emqtt:publish(
        C,
        bumblebee_connector:fill_pattern(PatPub, Vars),
        jsx:encode(Vars),
        default(QoS, 0)
    ).
...
encode_uplink(<<"json">>, Vars) ->
    jsx:encode(bumblebee_admin:build(Vars));
encode_uplink(<<"www-form">>, Vars) ->
    bumblebee_connector:form_encode(Vars).
...
auth_args(HostName, <<"sas">>, DeviceID, KeyName, SharedKey) ->
    ...
    nonempty_field(
        ...
        [
            ...
            {password,
                list_to_binary(
                    bumblebee_connector:shared_access_token(HostName, DeviceID, KeyName, SharedKey)
                )}
        ]
    );
% normal (and default)
...
ssl_args(mqtt, _Conn) ->
    [];
ssl_args(mqtts, #connector{certfile = CertFile, keyfile = KeyFile}) when
    is_binary(CertFile), size(CertFile) > 0, is_binary(KeyFile), size(KeyFile) > 0
->
    [
        {ssl, [
            {versions, ['tlsv1.2']},
...
build_status(Idx, Common, {[Uri, ClientId, _, _], _, Subs, _} = Item) ->
    ...
    Common2#{
        pid => bumblebee_connector:pid_to_binary(self(), Idx),
        uri => Uri,
        subs => Subs,
