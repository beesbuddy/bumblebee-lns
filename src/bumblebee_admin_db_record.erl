...
%
-module(bumblebee_admin_db_record).
...
init(Req, {Table, Fields, Scopes}) ->
    init0(Req, Table, Fields, bumblebee_admin, Scopes);
init(Req, {Table, Fields, Module, Scopes}) ->
    init0(Req, Table, Fields, Module, Scopes).

init0(Req, Table, Fields, Module, Scopes) ->
    Key = bumblebee_admin:parse_field(
        hd(Fields),
        cowboy_req:binding(id, Req, cowboy_req:binding(hd(Fields), Req))
    ),
    {cowboy_rest, Req, #state{
        table = Table, fields = Fields, key = Key, module = Module, scopes = Scopes
    }}.
...
is_authorized(Req, #state{scopes = Scopes} = State) ->
    case bumblebee_admin:handle_authorization_ex(Req, Scopes) of
...
forbidden(Req, #state{fields = [Key | _], auth_fields = {ReadFields, WriteFields}} = State) ->
    ...
    if
        Method == <<"OPTIONS">>; Method == <<"GET">> ->
            {bumblebee_admin:fields_empty(ReadFields), Req, State};
        Method == <<"DELETE">> ->
            % key must be writable
            {not bumblebee_admin:auth_field(Key, WriteFields), Req, State};
        true ->
            {bumblebee_admin:fields_empty(WriteFields), Req, State}
    end.
            ...
            maps:from_list(
                lists:filter(
                    fun
                        ...
                        ({pass_ha1, _}) -> false;
                        ({Name, _}) -> bumblebee_admin:auth_field(Name, ReadFields)
                    end,
...
handle_write(Req, #state{fields = Fields} = State) ->
    ...
    case catch jsx:decode(Data, [return_maps, {labels, atom}]) of
        Map when is_map(Map) ->
            ...
            Req3 = bumblebee_admin:set_body_json_id(hd(Fields), ID, Req2),
            {true, Req3, State};
...
get_db_field(pass_ha1, AuthFields, List2, Orig) ->
    case bumblebee_admin:auth_field(pass_ha1, AuthFields) of
        true ->
            % if password was not defined, use the previous value
            case maps:is_key(pass, List2) of
                true ->
                    bumblebee_http_digest:ha1({maps:get(name, List2), ?REALM, maps:get(pass, List2)});
                false ->
                    Orig
            end;
        false ->
...
get_db_field(Field, AuthFields, List2, Orig) ->
    case bumblebee_admin:auth_field(Field, AuthFields) of
        true ->
...
generate_etag(Req, #state{key = undefined} = State) ->
    {undefined, Req, State};
generate_etag(
    Req, #state{table = Table, fields = Fields, key = Key, auth_fields = {_, WriteFields}} = State
) ->
    case mnesia:dirty_read(Table, Key) of
        [] ->
            {undefined, Req, State};
        [Rec] ->
            ...
            FiltRec =
                lists:filter(
                    fun
                        ({Private, _}) when
                            Private == health_alerts;
                            Private == health_decay;
                            Private == health_reported;
                            Private == health_next
                        ->
                            false;
                        ({Name, _}) ->
                            bumblebee_admin:auth_field(Name, WriteFields)
                    end,
                    lists:zip(Fields, tl(tuple_to_list(Rec)))
                ),
...
delete_resource(Req, #state{table = Table, fields = Fields, key = Key} = State) ->
    Req2 = bumblebee_admin:set_body_json_id(hd(Fields), Key, Req),
