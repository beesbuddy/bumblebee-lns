...
valid_ha1(User, Pass, HA1) ->
    HA1 == lorawan_http_digest:ha1({User, ?REALM, Pass}) orelse
        HA1 == lorawan_http_digest:ha1({User, ?LEGACY_REALM, Pass}).
...
build_bitstring(Value) ->
    case bit_size(Value) rem 8 of
        0 ->
            #{
                val => lorawan_utils:binary_to_hex(Value),
                len => bit_size(Value)
            };
        N ->
            #{
                val => lorawan_utils:binary_to_hex(<<Value/bitstring, 0:(8 - N)>>),
                len => bit_size(Value)
            }
    end.
