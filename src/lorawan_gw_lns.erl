...
handle_message(#{router := Router}, #state{mac = undefined} = State) ->
    ...
    {ok, {Scheme, _, Host, Port, Path, _}} = lorawan_compat:parse_uri(Prefix, [
        {http, 80}, {https, 443}
    ]),
    Scheme2 =
        case Scheme of
    ...
    Res = #{
        ...
        uri => list_to_binary([
            ...
            Path,
            <<"router-info/">>,
            lorawan_utils:binary_to_hex(MAC)
        ])
    },
    {reply, Res, State};
handle_message(#{router := _Router}, #state{mac = MAC}) ->
    _ = lager:error("Unexpected router-info from ~s", [lorawan_utils:binary_to_hex(MAC)]),
    stop;
handle_message(
    #{
        msgtype := <<"version">>,
    ...
    _ = lager:debug("~s basicstation ~s ~s ~s ~s", [
        lorawan_utils:binary_to_hex(MAC), Station, Firmware, Package, Model
    ]),
...
handle_message(
    ...
    PHYPayload =
        <<MHdr, DevAddr:32/little-signed-integer, FCtrl, FCnt:16/little-unsigned-integer,
            (lorawan_utils:hex_to_binary(FOpts))/binary, FPort/binary,
            (lorawan_utils:hex_to_binary(FRMPayload))/binary, MIC:32/little-signed-integer>>,
    lorawan_gw_router:uplinks([{{MAC, RxQ, GWState}, PHYPayload}]);
handle_message(#{msgtype := <<"propdf">>, 'FRMPayload' := FRMPayload}, #state{mac = MAC}) ->
    lager:debug("~s got proprietary frame ~s", [
        lorawan_utils:binary_to_hex(MAC), lorawan_utils:binary_to_hex(FRMPayload)
    ]);
...
router_config(#area{region = Region}, MAC) ->
    ...
    lager:debug("gateway ~s init radio_0: ~p, radio_1: ~p", [
        lorawan_utils:binary_to_hex(MAC), Ra1Fr, Ra2Fr
    ]),
    #{
