    ...
    case catch jsx:decode(Data, [return_maps, {labels, atom}]) of
        ...
        _ ->
            lager:error("Ignored PUSH_DATA from ~s: JSON syntax error: ~s", [
                bumblebee_utils:binary_to_hex(MAC), Data
            ])
    end,
    {noreply, State};
% PULL DATA
handle_info(
    ...
    bumblebee_gw_router:alive(MAC, ?MODULE, {Host, Port, Version}),
    {noreply, State};
% TX ACK
handle_info(
    ...
    {DevAddr, Tokens2} =
        case maps:take(Token, Tokens) of
            {{Timer, AState, DStamp}, Tkns} ->
                ...
                bumblebee_gw_router:network_delay(MAC, AStamp - DStamp),
                {AState, Tkns};
            error ->
                {undefined, Tokens}
        end,
                ...
                Data2 when is_map(Data2) ->
                    ...
                    case maps:get(error, Ack, undefined) of
                        ...
                        <<"NONE">> ->
                            ok;
                        Error ->
                            bumblebee_gw_router:downlink_error(
                                MAC,
                                DevAddr,
...
status(MAC, Pk) ->
    bumblebee_gw_router:report(MAC, ?to_record(stat, Pk)).

rxpk(MAC, PkList) ->
    bumblebee_gw_router:uplinks(
        lists:map(
            fun(Pk) ->
