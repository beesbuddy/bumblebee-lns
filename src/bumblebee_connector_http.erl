...
%
-module(bumblebee_connector_http).
...
start_connector(#connector{connid = Id, received = Received} = Connector) ->
    case bumblebee_connector:pattern_for_cowboy(Received) of
        undefined ->
            ok;
        error ->
            bumblebee_connector:raise_failed(Id, {badarg, Received});
        Pattern ->
            bumblebee_http_registry:update(
                {http, Id},
                #{routes => [{Pattern, bumblebee_connector_http_in, [Connector]}]}
            )
    end,
    bumblebee_connector_sup:start_child(Id, ?MODULE, [Connector]).

stop_connector(Id) ->
    bumblebee_http_registry:delete({http, Id}),
    bumblebee_connector_sup:stop_child(Id).
        ...
        {ok,
            ensure_gun(
                #state{
                    ...
                    auth = bumblebee_connector:prepare_filling([UserName, Password]),
                    nc = 1
                }
            )}
    catch
...
handle_info(
    ...
    if
        ...
        true ->
            ...
            bumblebee_utils:throw_warning(connector_http, {http_error, {Status, Uri, Path}})
    end,
...
ensure_gun(#state{conn = #connector{connid = ConnId, uri = Uri}, pid = undefined} = State) ->
    ...
    {ConnPid, Prefix} =
        case bumblebee_compat:parse_uri(Uri, [{http, 80}, {https, 443}]) of
...
handle_uplink(Vars0, #state{conn = #connector{format = Format}, publish_uplinks = Publish} = State) ->
    ...
    send_publish(bumblebee_admin:build(Vars0), Publish, ContentType, Body, State).
...
send_publish(
    ...
    URI = binary:list_to_bin([Prefix | bumblebee_connector:fill_pattern(Publish, Vars)]),
    [User, Pass] = bumblebee_connector:fill_pattern(AuthP, Vars),
    case Conn of
...
encode_uplink(<<"json">>, Vars) ->
    {<<"application/json">>, jsx:encode(bumblebee_admin:build(Vars))};
encode_uplink(<<"www-form">>, Vars) ->
    {<<"application/x-www-form-urlencoded">>, bumblebee_connector:form_encode(Vars)}.
...
handle_authenticate0(basic, _, _, [Name, Pass], _, State) ->
    ...
    {[bumblebee_http_digest:authorization_header(basic, Cred)], State};
handle_authenticate0(digest, Value, URI, [Name, Pass], Body, State = #state{nc = Nc0}) ->
    ...
    case proplists:get_value(<<"qop">>, Value) of
        undefined ->
            Response = bumblebee_http_digest:response(
                <<"POST">>, URI, Body, {Name, Realm, Pass}, Nonce
            ),
            {
                [
                    bumblebee_http_digest:authorization_header(digest, [
                        {<<"username">>, Name},
                        {<<"realm">>, Realm},
                        {<<"nonce">>, Nonce},
                        {<<"uri">>, URI},
        ...
        Qop0 ->
            ...
            Nc = bumblebee_http_digest:nc(Nc0),
            CNonce = bumblebee_http_digest:nonce(4),
            Response = bumblebee_http_digest:response(
