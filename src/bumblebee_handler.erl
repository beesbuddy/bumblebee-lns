...
idle(cast, {frame, {MAC, RxQ, _}, <<2#000:3, _/bitstring>> = PHYPayload}, _Data) ->
    case bumblebee_mac:ingest_frame(MAC, PHYPayload) of
        {join, {Network, Profile, Device}, DevAddr, DevNonce} ->
            case invoke_handler(handle_join, {Network, Profile, Device}, [{MAC, RxQ}, DevAddr]) of
                ...
                ignore ->
                    {next_state, drop, []};
                {error, Error} ->
                    bumblebee_utils:throw_error({node, DevAddr}, Error),
                    {next_state, drop, []}
            end;
        ignore ->
            {next_state, drop, []};
        {error, Error} ->
            bumblebee_utils:throw_error(server, Error),
            {next_state, drop, []};
        {error, Object, Error} ->
            bumblebee_utils:throw_error(Object, Error),
            {next_state, drop, []};
        {error, Object, Error, Args} ->
            bumblebee_utils:throw_error(Object, Error, Args),
            {next_state, drop, []}
    end;
idle(cast, {frame, {MAC, RxQ, _}, PHYPayload}, _Data) ->
    ...
    case bumblebee_mac:ingest_frame(MAC, PHYPayload) of
        {uplink, {Network, Profile, #node{devaddr = DevAddr} = Node}, #frame{ack = ACK} = Frame} ->
            ...
            LastMissed =
                case mnesia:dirty_read(pending, DevAddr) of
                    [#pending{confirmed = true, receipt = Receipt}] when ACK == 0 ->
                        bumblebee_utils:throw_warning({node, DevAddr}, downlink_missed),
                        {missed, Receipt};
                    [#pending{confirmed = true, receipt = Receipt}] when ACK == 1 ->
                        ok = mnesia:dirty_delete(pending, DevAddr),
                        invoke_handler(handle_delivery, {Network, Profile, Node}, [
                            delivered, Receipt
                        ]),
                        undefined;
                    [_Msg] ->
            ...
            case
                invoke_handler(handle_uplink, {Network, Profile, Node}, [
                    {MAC, RxQ}, LastMissed, Frame
                ])
            of
                {ok, AppState} ->
                ...
                {error, Error} ->
                    bumblebee_utils:throw_error({node, Node#node.devaddr}, Error),
...
join(cast, {rxq, Gateways0}, {{Network, Profile, Device}, DevAddr, DevNonce}) ->
    ...
    {ok, #node{reset_count = JoinCnt} = Node, PHYPayload} =
        bumblebee_mac:handle_accept(Gateways, {Network, Profile, Device}, DevAddr, DevNonce),
    TxQ =
        case alternate_rxwin(Profile, JoinCnt) of
            0 ->
                lager:debug("Join-Accept in RX1: ~p", [RxQ]),
                bumblebee_mac_region:join1_window(Network, RxQ);
            1 ->
                lager:debug("Join-Accept in RX2: ~p ~p", [RxQ, Node#node.rxwin_use]),
                bumblebee_mac_region:join2_window(Network, Node)
        end,
    bumblebee_gw_router:downlink({MAC, GWState}, Network, DevAddr, TxQ, PHYPayload),
...
uplink(
    cast,
    {rxq, Gateways0},
    {TimeStamp, {Network, Profile, Node},
    ...
    {ok, MacConfirm, Node2, FOptsOut} =
        bumblebee_mac_commands:handle_fopts({Network, Profile, Node}, Gateways, ADR, FOpts),
    ...
    % invoke applications
    case
        invoke_handler(handle_rxq, {Network, Profile, Node2}, [
            Gateways, ShallReply, Frame, AppState
        ])
    of
        {send, TxData} ->
        ...
        ok when ShallReply ->
            ...
            send_unicast(
                {MAC, GWState}, {Network, Profile, Node2}, TxQ, Confirm, FOptsOut, #txdata{
                    data = <<>>
                }
            );
        ok ->
...
choose_tx({Network, #profile{txwin = 1}, Node}, RxQ, _Timestamp) ->
    bumblebee_mac_region:rx1_window(Network, Node, RxQ);
choose_tx({Network, #profile{txwin = 2}, Node}, _RxQ, _Timestamp) ->
    bumblebee_mac_region:rx2_window(Network, Node);
choose_tx({#network{rx1_delay = Rx1Delay} = Network, _Profile, Node}, RxQ, TimeStamp) ->
    {ok, GwDelay} = application:get_env('bumblebee_lns', gateway_delay),
...
send_unicast(
    {MAC, GWState},
    {Network, Profile, #node{devaddr = DevAddr} = Node},
    TxQ,
    ACK,
    FOpts,
    #txdata{confirmed = Confirmed, receipt = Receipt} = TxData
) ->
    {ok, Node2, PHYPayload} = bumblebee_mac:encode_unicast(
        {Network, Profile, Node}, ACK, FOpts, TxData
    ),
    % we are about to overwrite the pending frame for this device
    case mnesia:dirty_read(pending, DevAddr) of
        [#pending{confirmed = true, receipt = Receipt}] ->
            bumblebee_utils:throw_error({node, DevAddr}, downlink_expired),
...
retransmit(cast, {rxq, Gateways0}, {{Network, Profile, #node{devaddr = DevAddr} = Node}, Frame}) ->
    ...
    case ToTransmit of
        {PHYPayload, Count} ->
            ...
            ok = mnesia:dirty_write(
                build_rxframe(<<"re-up">>, Gateways, {Network, Profile, Node}, Frame)
            ),
            TxQ =
                case alternate_rxwin(Profile, Count) of
                    0 -> bumblebee_mac_region:rx1_window(Network, Node, RxQ);
                    1 -> bumblebee_mac_region:rx2_window(Network, Node)
                end,
            lager:debug("~s retransmitting ~B", [bumblebee_utils:binary_to_hex(DevAddr), Count]),
            bumblebee_gw_router:downlink({MAC, GWState}, Network, DevAddr, TxQ, PHYPayload),
            {stop, normal, []};
        undefined ->
...
invoke_handler(Fun, {_, #profile{app = App}, _} = Subject, Params) ->
    ...
    case proplists:get_value(App, Modules) of
        undefined ->
            ...
            apply(bumblebee_application_backend, Fun, [Subject | Params]);
        {_, Module} ->
...
downlink({Network, Profile, #node{gateways = [{MAC, _RxQ} | _]} = Node}, Time, TxData) ->
    TxQ = bumblebee_mac_region:rx2_rf(Network, Node),
    ...
    send_unicast(
        ...
        TxQ#txq{time = Time},
        0,
        bumblebee_mac_commands:build_fopts({Network, Profile, Node}, []),
        TxData
    );
downlink({_, _, #node{devaddr = DevAddr}}, _, _) ->
    bumblebee_utils:throw_error({node, DevAddr}, no_uplink_yet).

multicast(
    #multicast_channel{devaddr = DevAddr, profiles = Profiles} = Channel,
    Time,
    #txdata{confirmed = false} = TxData
) ->
    % must be unconfirmed, ACK=0, no MAC commands allowed
    {ok, _, PHYPayload} = bumblebee_mac:encode_multicast(DevAddr, TxData),
    lists:foreach(
        fun(ProfName) ->
            ...
            ok = mnesia:dirty_write(
                build_rxframe(<<"bcast">>, Gateways, {Network, Profile, Channel}, TxData)
            ),
            TxQ = bumblebee_mac_region:rx2_rf(Network, Profile),
            lists:foreach(
                fun(MAC) ->
                    bumblebee_gw_router:downlink(
                        {MAC, undefined},
                        Network,
                        DevAddr,
                        TxQ#txq{time = Time},
...
multicast(#multicast_channel{devaddr = DevAddr}, _Time, #txdata{confirmed = true}) ->
    bumblebee_utils:throw_error({multicast_channel, DevAddr}, confirmed_not_allowed).
    ...
    lists:usort(
        lists:filtermap(
            ...
            mnesia:dirty_select(node, [
                {#node{profile = '$1', gateways = '$2', _ = '_'}, [{'==', '$1', ProfName}], ['$2']}
            ])
        )
    ).
...
build_rxframe(
    Dir,
    Gateways,
    {#network{name = NetName}, #profile{app = App}, #node{
        location = Location, fcntup = FCnt, average_qs = AverageQs, adr_use = {TXPower, _, _}
    }},
    ...
    #rxframe{
        ...
        location = Location,
        gateways = Gateways,
        average_qs = AverageQs,
        powe = TXPower,
        fcnt = FCnt,
        confirm = bit_to_bool(Confirm),
        port = Port,
        data = Data,
        datetime = bumblebee_utils:precise_universal_time()
    };
build_rxframe(
    Dir,
    MAC,
    {#network{name = NetName}, #profile{app = App}, #node{
        location = Location, devaddr = DevAddr, fcntdown = FCnt
    }},
...
build_rxframe(
    Dir,
    MAC,
    {#network{name = NetName}, #profile{app = App}, #multicast_channel{
        devaddr = DevAddr, fcntdown = FCnt
    }},
    #txdata{confirmed = Confirm, port = Port, data = Data}
) ->
    {ok, FrId} = eid:get_bin(),
