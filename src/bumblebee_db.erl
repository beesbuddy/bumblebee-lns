...
ensure_indexes(Name, TabDef) ->
    ...
    NewIndexes =
        lists:sort(
            lists:map(
                fun(Key) ->
                    bumblebee_utils:index_of(Key, NewAttrs) + 1
                end,
        ...
        {ok, {User, Pass}} ->
            ...
            case mnesia:dirty_read(user, User) of
                [#user{pass_ha1 = HA1}] ->
                    ok;
                [U = #user{}] ->
                    lager:info("Updating configured admin credentials for ~s", [User]),
                    mnesia:dirty_write(U#user{pass_ha1 = HA1});
                [] ->
                    lager:info("Creating configured admin user ~s", [User]),
                    mnesia:dirty_write(#user{name = User, pass_ha1 = HA1})
            end;
        undefined ->
            ok
    end.
...
join_cluster(MasterNode, SlaveNode) ->
    ...
    case {MasterNode, SlaveNode} of
        ...
        {Self, SlaveNode} ->
            ...
            rpc:call(SlaveNode, bumblebee_db, join, [node()], 10000)
    end.
