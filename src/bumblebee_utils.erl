...
throw_event(Severity, {Entity, undefined}, Text, Mark) ->
    ...
    bumblebee_prometheus:event(Severity, {Entity, undefined}, Text),
...
throw_event(Severity, {Entity, EID}, Text, Mark) ->
    if
        ...
        true ->
            lager:log(Severity, self(), "~s ~s ~p", [
                Entity, bumblebee_utils:binary_to_hex(EID), Text
            ])
    end,
    bumblebee_prometheus:event(Severity, {Entity, EID}, Text),
...
send_event(Severity, {Entity, EID}, Text) ->
    ...
    bumblebee_backend_factory:event(AppID, undefined, Vars).
