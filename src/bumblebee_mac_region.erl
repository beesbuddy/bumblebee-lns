...
rx2_rf(#network{region = Region, tx_codr = CodingRate, rxwin_init = WinInit}, #profile{
    rxwin_set = WinSet
}) ->
    {_, DataRate, Freq} = bumblebee_mac_commands:merge_rxwin(WinSet, WinInit),
    #txq{freq = Freq, datr = dr_to_datar(Region, DataRate), codr = CodingRate}.
...
uch2f(Region, Ch) when
    (Region == <<"US902">> orelse Region == <<"US902-PR">>) andalso Ch < 64
->
    ch2fi(Ch, {9023, 2});
uch2f(Region, Ch) when
    Region == <<"US902">> orelse Region == <<"US902-PR">>
->
    ch2fi(Ch - 64, {9030, 16});
uch2f(<<"AU915">>, Ch) when
    Ch < 64
...
net_freqs(#network{region = Region, init_chans = Chans}) when
    Region == <<"US902">>; Region == <<"US902-PR">>; Region == <<"AU915">>; Region == <<"CN470">>
->
...
bits_test_() ->
    [
        ...
        ?_assertEqual(
            [{link_adr_req, <<"SF12BW250">>, 14, 7, 0, 0}],
            set_channels(<<"EU868">>, {14, <<"SF12BW250">>, [{0, 2}]}, [])
        ),
        ?_assertEqual(
            [
                {link_adr_req, <<"SF12BW500">>, 20, 0, 7, 0},
                {link_adr_req, <<"SF12BW500">>, 20, 255, 0, 0}
            ],
            set_channels(<<"US902">>, {20, <<"SF12BW500">>, [{0, 7}]}, [])
        ),
        ?_assertEqual(
            [
                {link_adr_req, <<"SF12BW500">>, 20, 2, 7, 0},
                {link_adr_req, <<"SF12BW500">>, 20, 65280, 0, 0}
            ],
