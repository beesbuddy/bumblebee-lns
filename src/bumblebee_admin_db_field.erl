...
%
-module(bumblebee_admin_db_field).
...
init0(Req, Table, Fields, Module, Scopes) ->
    Key = bumblebee_admin:parse_field(hd(Fields), cowboy_req:binding(hd(Fields), Req)),
    Field = binary_to_existing_atom(cowboy_req:binding(field, Req), latin1),
    {cowboy_rest, Req, #state{
        table = Table,
        key = Key,
        field = Field,
        fidx = bumblebee_utils:index_of(Field, Fields),
        module = Module,
        scopes = Scopes
    }}.
...
forbidden(Req, #state{field = Field, auth_fields = AuthFields} = State) ->
    {not bumblebee_admin:auth_field(Field, AuthFields), Req, State}.
