...
%
-module(bumblebee_admin_graph_node).
...
get_rxframe(Req, State) ->
    ...
    [Node] = mnesia:dirty_read(node, bumblebee_utils:hex_to_binary(DevAddr)),
...
get_array(#node{last_reset = Reset, devstat = DevStat}) when is_list(DevStat) ->
    ...
    [
        ...
        {rows, [
            [
                {c, [
                    [{v, bumblebee_admin:timestamp_to_json_date(Reset)}],
                    [{v, <<"Last Reset">>}],
                    [{v, null}],
                    [{v, null}],
                    [{v, null}]
            ...
            | lists:map(
                fun
                    ({Timestamp, Batt, Margin, MaxSNR}) ->
                        [
                            {c, [
                                [{v, bumblebee_admin:timestamp_to_json_date(Timestamp)}],
                                [{v, null}],
                                [{v, Batt}],
...
generate_etag(Req, State) ->
    DevAddr = cowboy_req:binding(devaddr, Req),
    case mnesia:dirty_read(node, bumblebee_utils:hex_to_binary(DevAddr)) of
        [] ->
            {undefined, Req, State};
