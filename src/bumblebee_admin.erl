...
handle_authentication_field(_Req, {basic, User, Pass}) ->
    case load_user_credentials(User) of
        {ok, HA1, AuthScopes} ->
...
handle_authentication_field(Req, {digest, Params}) ->
    ...
    case load_user_credentials(UserName) of
        {ok, HA1, AuthScopes} ->
            case bumblebee_http_digest:response(Method, URI, <<>>, HA1, Nonce) of
                Response ->
                    {true, AuthScopes};
                _Else ->
                    {false, basic_header()}
...
handle_authorization_ex(Req, {Read, Write}) ->
    case handle_authentication(Req) of
        ...
        {true, AuthScopes} ->
            {true, authorized_fields(AuthScopes, Read ++ Write),
                authorized_fields(AuthScopes, Write)};
        {false, Header} ->
            {false, Header}
    end;
...
load_user_credentials(User) ->
    case catch mnesia:dirty_read(user, User) of
        [#user{pass_ha1 = HA1, scopes = AuthScopes}] ->
            {ok, HA1, normalize_auth_scopes(User, AuthScopes)};
        [] ->
...
normalize_auth_scopes(_User, AuthScopes) ->
    AuthScopes.
...
write(Rec) ->
    mnesia:write(bumblebee_db_guard:update_health(Rec)).
...
parse_eid(Value, #{entity := Entity}) when Entity == server; Entity == connector ->
    Value;
parse_eid(Value, _) ->
    bumblebee_utils:hex_to_binary(Value).

parse_field(_Key, Value) when Value == null; Value == undefined ->
    undefined;
parse_field(Key, Value) when
    Key == mac;
    Key == netid;
    Key == mask;
    Key == deveui;
...
parse_field(Key, Value) when Key == gateways ->
    lists:map(
        fun(#{mac := MAC, rxq := RxQ}) ->
            {bumblebee_utils:hex_to_binary(MAC), ?to_record(rxq, parse(RxQ))}
        end,
        Value
    );
...
parse_field(Key, Value) when Key == last_joins ->
    lists:map(
        fun(#{time := Time, dev_nonce := DevNonce}) ->
            {iso8601:parse(Time), bumblebee_utils:hex_to_binary(DevNonce)}
        end,
        Value
    );
parse_field(Key, Value) when
...
build_eid(undefined, _) ->
    null;
build_eid(Value, #{entity := Entity}) when
    Entity == server; Entity == connector; Entity == handler
->
    Value;
build_eid(Value, _) ->
    bumblebee_utils:binary_to_hex(Value).

build_field(_Key, undefined) ->
    null;
build_field(Key, Value) when
    Key == mac;
    Key == netid;
    Key == mask;
    Key == deveui;
...
build_field(Key, Value) when Key == gateways ->
    lists:map(
        fun({MAC, RxQ}) ->
            #{mac => bumblebee_utils:binary_to_hex(MAC), rxq => build(?to_map(rxq, RxQ))}
        end,
        Value
    );
...
build_field(Key, Value) when Key == last_joins ->
    lists:map(
        fun({Time, DevNonce}) ->
            #{time => iso8601:format(Time), dev_nonce => bumblebee_utils:binary_to_hex(DevNonce)}
        end,
        Value
    );
build_field(Key, Value) when
...
parse_bitstring(Map) ->
    case parse_opt(val, Map) of
        ...
        Val ->
            BinVal = bumblebee_utils:hex_to_binary(Val),
            case parse_opt(len, Map) of
                undefined ->
                    BinVal;
...
build_bitstring(Value) ->
    case bit_size(Value) rem 8 of
        0 ->
            #{
                val => bumblebee_utils:binary_to_hex(Value),
                len => bit_size(Value)
            };
        N ->
            #{
                val => bumblebee_utils:binary_to_hex(<<Value/bitstring, 0:(8 - N)>>),
                len => bit_size(Value)
            }
    end.
...
build_adr({TXPower, DataRate, Chans}) ->
    #{
        power => build_opt(TXPower),
        datr => build_opt(DataRate),
        chans =>
            case Chans of
                undefined -> null;
...
parse_rxwin(List) ->
    {parse_opt(rx1_dr_offset, List), parse_opt(rx2_dr, List), parse_opt(rx2_freq, List)}.

build_rxwin({RX1DROffset, RX2DataRate, Frequency}) ->
    #{
        rx1_dr_offset => build_opt(RX1DROffset),
        rx2_dr => build_opt(RX2DataRate),
        rx2_freq => build_opt(Frequency)
    };
build_rxwin(_Else) ->
    #{}.
...
build_devstat({Battery, Margin}) ->
    [
        #{
            ...
            battery => build_opt(Battery),
            margin => build_opt(Margin),
            max_snr => 0
        }
    ];
build_devstat(Value) ->
    lists:map(
        fun
            ({Timestamp, Battery, Margin, MaxSNR}) ->
                #{
                    datetime => iso8601:format(Timestamp),
                    battery => Battery,
                    margin => Margin,
                    max_snr => MaxSNR
                };
...
%% temporary db consistency fix, to be removed after some time
build_opt(<<"undefined">>) -> null;
...
bits_test_() ->
    [
        ...
        ?_assertEqual([{0, 7}, {64, 64}], text_to_intervals("0-7,64")),
