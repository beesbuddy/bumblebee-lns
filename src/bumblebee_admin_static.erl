...
priv_dir(App) ->
    case code:priv_dir(App) of
        {error, bad_name} ->
            case fallback_priv_dir() of
                undefined ->
                    _ = lager:error("Unknown priv_dir of ~p", [App]),
                    [];
                Dir ->
                    _ = lager:warning("Falling back to inferred priv_dir ~s for ~p", [Dir, App]),
                    Dir
            end;
        PrivDir ->
            PrivDir
