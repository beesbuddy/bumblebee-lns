            ...
            [
                {
                    ...
                    [
                        {'andalso', {'is_tuple', '$2'},
                            {'<', '$2', {const, calendar:universal_time()}}}
                    ],
                    ['$1']
                }
            ]
...
update_health(
    #gateway{
        ...
        area = Area,
        dwell = Dwell,
        health_alerts = Alerts0,
        health_reported = Reported0
    } = Gateway0
) ->
    % expire old data
    Gateway = Gateway0#gateway{dwell = bumblebee_gw_router:update_dwell([], Dwell)},
...
update_health(
    ...
    {Reports, Alerts, Decay, Reported, MinNext} =
        check_health(
            ...
            Reported0,
            ?MODULE,
            [check_reset, check_battery, check_margin, check_adr, check_rxwin]
        ),
    case Reports of
        {NewAlerts, OtherAlerts} ->
            case bumblebee_db:get_group(Node) of
                #group{admins = Admins, slack_channel = Channel} ->
                    send_alert(
                        Admins,
                        Channel,
                        "node",
                        bumblebee_utils:binary_to_hex(DevAddr),
                        NewAlerts,
                        OtherAlerts,
                        Decay
                    );
        ...
        {NewAlerts, OtherAlerts} ->
            AffectedGroups =
                lists:usort(
                    [
                        Gr
                     || #profile{group = Gr} <- mnesia:dirty_index_read(
                            profile, AppId, #profile.app
                        )
                    ]
                ),
            lists:foreach(
                fun(Group) ->
                    case mnesia:dirty_read(group, Group) of
                        #group{admins = Admins, slack_channel = Channel} ->
                            send_alert(
                                Admins, Channel, "connector", ConnId, NewAlerts, OtherAlerts, Decay
                            );
                        _Else ->
                            ok
                    end
        ...
        lists:filtermap(
            fun(User) ->
                case mnesia:dirty_read(user, User) of
                    [#user{email = EMail, send_alerts = true}] when
                        is_binary(EMail), byte_size(EMail) > 0
                    ->
                        {true, EMail};
                    _Else ->
                        false
...
send_emails0(
    ...
    byte_size(From) > 0, byte_size(Server) > 0, length(ToAddrs) > 0
->
    ...
    gen_smtp_client:send(
        {From, ToAddrs, mimemail:encode(Body)},
        [{relay, Server}] ++
            if
...
check_reset(#node{last_reset = LastRes, reset_count = Count, last_rx = LastRx}) when
    LastRes /= undefined, is_number(Count), Count > 1, (LastRx == undefined orelse LastRes > LastRx)
->
...
check_margin(#node{devstat = DevStat}) when is_list(DevStat), length(DevStat) > 1 ->
    ...
    case
        lists:all(
            fun(Delta) -> Delta < Threshold end, Deltas
        )
    of
        true ->
...
node_deleted(DevAddr) ->
    _ = lager:debug("Node ~p deleted", [bumblebee_utils:binary_to_hex(DevAddr)]),
...
trim_rxframes() ->
    ...
    if
        length(Trimmed) > 0 ->
            lager:debug(
                "Expired rxframes from ~p",
                [[bumblebee_utils:binary_to_hex(E) || E <- Trimmed]]
            );
        true ->
            ok
    end.
