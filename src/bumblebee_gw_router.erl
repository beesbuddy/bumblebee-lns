...
downlink(
    {MAC, GWState}, #network{gw_power = DefPower, max_eirp = MaxEIRP}, DevAddr, TxQ, PHYPayload
) ->
    ...
    gen_server:cast(
        ?MODULE, {downlink, {MAC, GWState}, DevAddr, TxQ#txq{powe = Power}, RFCh, PHYPayload}
    ).
...
init([]) ->
    {ok, Interval} = application:get_env('bumblebee_lns', server_stats_interval),
    ...
    Stats =
        case dict:find(MAC, Dict) of
            {ok, #gwstats{process = Process, target = Target} = S} ->
                S;
            {ok, #gwstats{} = S} ->
                bumblebee_utils:throw_info({gateway, MAC}, {connected, {Host, Port}}),
...
handle_cast({uplinks, PkList}, State) ->
    ...
    {noreply, State2};
handle_cast(
    {downlink, {MAC, GWState}, DevAddr, TxQ, RFCh, PHYPayload}, #state{gateways = Dict} = State
) ->
    ...
    case dict:find(MAC, Dict) of
        {ok, #gwstats{process = Process, target = Target, tx_times = TxTimes} = Stats} ->
            ...
            % store statistics
            bumblebee_prometheus:downlink(MAC),
            Time = bumblebee_mac_region:tx_time(byte_size(PHYPayload), TxQ),
            Dict2 = dict:store(
                MAC, Stats#gwstats{tx_times = [{TxQ#txq.freq, Time} | TxTimes]}, Dict
            ),
            {noreply, State#state{gateways = Dict2}};
        error ->
            lager:warning("Downlink request ignored. Gateway ~s not connected.", [
                bumblebee_utils:binary_to_hex(MAC)
            ]),
            {noreply, State}
    end;
handle_cast({downlink_error, MAC, undefined, Error}, #state{error_cnt = Cnt} = State) ->
    bumblebee_utils:throw_error({gateway, MAC}, Error),
...
terminate(Reason, _State) ->
    ...
    _ = lager:info("gateway router terminated: ~p", [Reason]),
    ok.
...
handle_alive(MAC, #gwstats{
    target = Target, last_gps = LastGPS, tx_times = TxTimes, nwk_delays = NwkDelays
}) ->
    {atomic, ok} = mnesia:transaction(
        fun() ->
            case mnesia:read(gateway, MAC, write) of
                [#gateway{dwell = Dwell, delays = Delay} = G] ->
                    bumblebee_admin:write(
                        G#gateway{
                            ip_address = Target,
...
update_dwell(TxTimes, Dwell0) ->
    ...
    Now = bumblebee_utils:precise_universal_time(),
    HourAgo = bumblebee_utils:apply_offset(Now, {-1, 0, 0}),
...
append_delays(NwkDelays, Delay) ->
    case NwkDelays of
        List when length(List) > 0 ->
            New = {
                lists:min(NwkDelays),
                round(lists:sum(NwkDelays) / length(NwkDelays)),
                lists:max(NwkDelays)
            },
...
handle_report(MAC, S) ->
    if
        S#stat.rxfw < S#stat.rxok ->
            bumblebee_utils:throw_warning(
                {gateway, MAC}, {uplinks_lost, S#stat.rxok - S#stat.rxfw}
            );
        true ->
            ok
    end,
    if
        S#stat.rxfw > 0, S#stat.ackr < 100 ->
            ...
            bumblebee_utils:throw_warning({gateway, MAC}, {ack_lost, 100 - S#stat.ackr});
        true ->
            ok
    end,
    if
        S#stat.txnb < S#stat.dwnb ->
            bumblebee_utils:throw_warning(
                {gateway, MAC}, {downlinks_lost, S#stat.dwnb - S#stat.txnb}
            );
        true ->
            ok
    end,
            ...
            case mnesia:read(gateway, MAC, write) of
                [G] ->
                    bumblebee_admin:write(
                        store_pos(
                            store_desc(
                                G#gateway{last_report = calendar:universal_time()}, S
                            ),
                            S
                        )
                    );
...
handle_uplink({{MAC, RxQ, _GWState} = GWData, PHYPayload}, State) ->
    case mnesia:dirty_read(gateway, MAC) of
        [] ->
            ...
            State;
        [_Gateway] ->
            bumblebee_prometheus:uplink(MAC),
...
handle_uplink0({GWData, PHYPayload}, #state{recent = Recent, request_cnt = Cnt} = State) ->
    case dict:find(PHYPayload, Recent) of
        error ->
            ...
            {ok, Handler} = bumblebee_handler_sup:start_child(),
            ...
            {ok, Delay} = application:get_env('bumblebee_lns', deduplication_delay),
            {ok, _} = timer:send_after(Delay, {rxq_ready, PHYPayload}),
            State#state{
                recent = dict:store(PHYPayload, {[GWData], Handler}, Recent), request_cnt = Cnt + 1
            };
