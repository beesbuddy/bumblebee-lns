...
%
-module(bumblebee_connector_amqp).
...
start_connector(#connector{connid = Id} = Connector) ->
    bumblebee_connector_sup:start_child({amqp, Id}, ?MODULE, [Connector]).

stop_connector(Id) ->
    bumblebee_connector_sup:stop_child({amqp, Id}).
...
init([
    #connector{
        connid = Id, app = App, publish_uplinks = PubUp, publish_events = PubEv, received = Cons
    } = Connector
]) ->
...
handle_info(
    connect,
    #state{conn = #connector{connid = ConnId, uri = Uri, name = UserName, pass = Password}} = State
) ->
    ...
    case amqp_uri:parse(Uri) of
        {ok, Params} ->
            ...
            of
                ...
                {error, Error} ->
                    bumblebee_connector:raise_failed(ConnId, {network, Error}),
                    {stop, shutdown, State}
            end;
        {error, Error} ->
            bumblebee_connector:raise_failed(ConnId, {badarg, Error}),
            {stop, shutdown, State}
    end;
handle_info(subscribe, #state{conn = #connector{subscribe = Sub}, cpid = Connection} = State) when
    is_pid(Connection), is_binary(Sub), byte_size(Sub) > 0
->
...
handle_info(
    {#'basic.deliver'{delivery_tag = Tag, exchange = Exchange, routing_key = RoutingKey}, Message},
    ...
    case
        bumblebee_connector:decode_and_downlink(
            Connector,
            Payload,
            bumblebee_connector:match_vars(Topic, Pattern)
        )
    of
        ok ->
            ok;
...
handle_info(
    {'DOWN', _Ref, process, SubChannel, {shutdown, {connection_closing, _}}},
    #state{subc = SubChannel} = State
) ->
...
handle_subscribe(
    #state{conn = #connector{connid = ConnId, subscribe = Sub}, subc = SubChannel} = State
) ->
    ...
    try amqp_channel:call(SubChannel, #'queue.declare'{auto_delete = true}) of
        #'queue.declare_ok'{queue = Queue} ->
            ...
            try amqp_channel:call(SubChannel, BindCmd) of
                ...
                _:{{shutdown, {_, _, Error}}, _} ->
                    bumblebee_connector:raise_failed(ConnId, {topic, Error}),
                    {stop, shutdown, State}
            end
    catch
        _:{{shutdown, {_, _, Error}}, _} ->
...
publish_uplink(PubChannel, Format, PatPub, Vars0) ->
    amqp_channel:cast(
        PubChannel,
        basic_publish(PatPub, bumblebee_admin:build(Vars0)),
...
basic_publish(PatPub, Vars) ->
    {Exchange, RoutingKey} =
        case binary:split(bumblebee_connector:fill_pattern(PatPub, Vars), <<$/>>) of
            [EX, RK | _] ->
                {EX, RK};
            [RK] ->
                {<<"amq.topic">>, RK}
...
set_subs(#state{conn = #connector{subscribe = Sub}, subc = SubChannel}, Map) when
    is_pid(SubChannel)
->
    Map#{subs => [Sub]};
