...
start(_Type, _Args) ->
    ...
    ok = log(debug, "Using config: ~p", [application:get_all_env(bumblebee)]),
    ...
    case
        start_listeners(
            application:get_env(bumblebee, http_admin_listen, []),
            retrieve_valid_ssl(),
            FrontendRoot,
            UseDevServer
        )
...
normal_dispatch(FrontendRoot, UseDevServer) ->
    cowboy_router:compile([
        {'_',
            lorawan_http_registry:get_static(routes) ++ lorawan_http_registry:get_custom(routes) ++
                lorawan_http_registry:get_frontend(FrontendRoot, UseDevServer)}
    ]).
