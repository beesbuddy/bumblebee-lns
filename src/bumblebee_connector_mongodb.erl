...
%
-module(bumblebee_connector_mongodb).
...
start_connector(#connector{connid = Id} = Connector) ->
    bumblebee_connector_sup:start_child({mongodb, Id}, ?MODULE, [Connector]).

stop_connector(Id) ->
    bumblebee_connector_sup:stop_child({mongodb, Id}).
...
store_fields(Pool, Pattern, Vars0) ->
    Target = bumblebee_connector:fill_pattern(Pattern, bumblebee_admin:build(Vars0)),
    {Database, Collection} =
