...
handle_fopts({Network, Profile, Node}, Gateways, ADR, FOpts) ->
    ...
    case FOptsIn of
        [] -> ok;
        List1 -> lager:debug("~s -> ~w", [bumblebee_utils:binary_to_hex(Node#node.devaddr), List1])
    end,
...
handle_fopts0({Network, Profile, Node0}, Gateways, FOptsIn) ->
    ...
    {LastQs, AverageQs} = append_qs(
        {RxQ#rxq.rssi, RxQ#rxq.lsnr}, Node1#node.last_qs, FramesRequired
    ),
...
build_fopts({Network, Profile, Node}, FOptsOut0) ->
    ...
    case FOptsOut of
        [] -> ok;
        List2 -> lager:debug("~s <- ~w", [bumblebee_utils:binary_to_hex(Node#node.devaddr), List2])
    end,
...
parse_fopts(<<>>) ->
    [];
parse_fopts(Unknown) ->
    lager:warning("Unknown command ~p", [bumblebee_utils:binary_to_hex(Unknown)]),
    [].
...
store_actual_adr(
    ...
    % store parameters
    DataRate = bumblebee_mac_region:datar_to_dr(Region, RxQ#rxq.datr),
            ...
            case merge_adr(Node#node.adr_set, Node#node.adr_use) of
                Unchanged when Unchanged == Node#node.adr_use ->
                    lager:debug(
                        ...
                        [bumblebee_utils:binary_to_hex(Node#node.devaddr)]
                    ),
...
handle_dcycle(FOptsIn, Profile, Node) ->
    case lists:member(duty_cycle_ans, FOptsIn) of
        true ->
            lager:debug("DutyCycleAns ~s", [bumblebee_utils:binary_to_hex(Node#node.devaddr)]),
...
handle_status(FOptsIn, #network{region = Region}, Node) ->
    case find_status(FOptsIn) of
        {Battery, Margin} ->
            ...
            MaxSNR = bumblebee_mac_region:max_downlink_snr(Region, DataRate, OffUse),
            ...
            Node#node{
                ...
                devstat = append_status(
                    {calendar:universal_time(), Battery, Margin, MaxSNR}, Node#node.devstat
                )
            };
        undefined ->
            Node
...
send_link_check([{_MAC, RxQ} | _] = Gateways) ->
    ...
    Margin = trunc(SNR - bumblebee_mac_region:max_uplink_snr(DataRate)),
...
send_device_time([{_MAC, #rxq{time = undefined}} | _]) ->
    MsSinceEpoch = bumblebee_utils:time_to_gps(),
...
send_device_time([{_MAC, #rxq{time = Time, tmms = undefined}} | _]) ->
    MsSinceEpoch = bumblebee_utils:time_to_gps(Time),
...
auto_adr(
    Network, #profile{adr_mode = 1} = Profile, #node{adr_flag = 1, adr_failed = Failed} = Node
) when
...
auto_adr(
    _Network, #profile{adr_mode = 2} = Profile, #node{adr_flag = 1, adr_failed = Failed} = Node
) when
    Failed == undefined; Failed == []
->
...
calculate_adr(
    ...
    MaxSNR = bumblebee_mac_region:max_uplink_snr(Region, DataRate) + 10,
...
->
    ...
    bumblebee_mac_region:set_channels(Region, Set, FOptsOut);
...
command_test_() ->
    [
        ...
        ?_assertEqual(
            ...
            parse_fopts(
                <<16#02, 16#03, 16#05, 16#04, 16#05, 16#07, 16#06, 100, 16#0A, 16#07, 16#03, 16#08,
                    16#09, 16#0A, 16#03, 16#0D>>
            )
        ),
        ?_assertEqual(
            <<16#02, 16#01, 16#02, 16#03, 16#34, 16#FF, 16#00, 16#21, 16#04, 16#05, 16#08, 16#07,
                16#09, 16#20, 16#0A, 16#01, 16#18, 16#4F, 16#84, 16#0D, 16#B0, 16#AD, 16#E8, 16#43,
                16#00>>,
            encode_fopts([
                {link_check_ans, 1, 2},
