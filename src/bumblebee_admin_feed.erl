...
init(Req, {Table, Fields, AuthFields}) ->
    init0(Req, Table, Fields, bumblebee_admin, AuthFields);
...
websocket_init(#state{table = Table} = State) ->
    ok = bumblebee_compat:pg_join({feed, Table}, self()),
...
notify(Scope) ->
    ...
    case bumblebee_compat:pg_get_members({feed, Table}) of
        {error, _Error} ->
            ok;
...
build_record(Rec, #state{fields = Fields, module = Module, auth_fields = AuthFields}) ->
    apply(Module, build, [
        maps:from_list(
            lists:filter(
                fun({Name, _}) ->
                    bumblebee_admin:auth_field(Name, AuthFields)
                end,
