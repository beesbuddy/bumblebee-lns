...
connection_for_node(Node, #state{connect = PatConn, hier = Hier}) ->
    Connect = lorawan_connector:fill_pattern(
        PatConn,
...
auth_args(HostName, <<"sas">>, DeviceID, KeyName, SharedKey) ->
    ...
    nonempty_field(
        ...
        [
            ...
            {password,
                list_to_binary(
                    lorawan_connector:shared_access_token(HostName, DeviceID, KeyName, SharedKey)
                )}
        ]
    );
% normal (and default)
...
build_status(Idx, Common, {[Uri, ClientId, _, _], _, Subs, _} = Item) ->
    ...
    Common2#{
        pid => lorawan_connector:pid_to_binary(self(), Idx),
        uri => Uri,
        subs => Subs,
