...
%
-module(bumblebee_admin_graph_gw).
...
get_gateway(Req, #state{format = Type} = State) ->
    ...
    [Gateway] = mnesia:dirty_read(gateway, bumblebee_utils:hex_to_binary(MAC)),
            ...
            lists:filtermap(
                fun
                    ({Date, {Min, Avg, Max}}) ->
                        {true, [
                            {c, [
                                [{v, bumblebee_admin:timestamp_to_json_date(Date)}],
                                [{v, Avg}],
                                [{v, Min}],
                                [{v, Max}]
                            ]}
...
resource_exists(Req, State) ->
    case
        mnesia:dirty_read(
            gateway,
            bumblebee_admin:parse_field(mac, cowboy_req:binding(mac, Req))
        )
    of
        [] -> {false, Req, State};
